...
//         setAppInstalled(false);
//       }, 2000); // Set a timeout to handle if the app doesn't respond
      
//       // Attempt to open the app
//       openApp();

//       // Clear the timeout if the app responds
...
export default function Home() {
  const [appInstalled, setAppInstalled] = useState(false);
  const appScheme = 'myapp://'; // Replace with your app scheme
  const appPackageName = 'com.thebrassstargroup.rtduserapp'; // Package name of the app
  const appStoreUrl = `https://play.google.com/store/apps/details?id=${appPackageName}`;
  const appStoreUrliOS = `https://apps.apple.com/app/${appPackageName}`; // Modify this with the correct App Store URL
  ...
  useEffect(() => {
    ...
    const checkAppInstalled = () => {
      ...
      if (window.navigator && window.navigator.standalone) {
        setAppInstalled(true); // iOS
      } else if (window.document && window.document.URL.indexOf(appScheme) !== -1) {
        setAppInstalled(true); // Android
      } else {
        setAppInstalled(false);
      }
    };
